...
export const loadThemesAtom = atom(null, async (get, set) => {
    ...
    try {
        const fetchData: FetchResult<SettingsThemeSchema> = await (await fetchGetData(`${API_ROUTE}${SETTINGS_ROUTE}/themes`))
...
export const setThemeAtom = atom(null, async (get, set, id: number) => {
    ...
    try {
        const result = await fetchData(`${API_ROUTE}${SETTINGS_ROUTE}/theme`, "PUT", JSON.stringify({ id }))
        set(loadThemesAtom)
