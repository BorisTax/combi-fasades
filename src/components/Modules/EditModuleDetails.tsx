...
const emptyDetail: ModuleDetailsTableSchema = { id: 0, name: "", matIndex: 0, length: 0, width: 0, commentId: 0, grooveId: 0, el1: 0, el2: 0, ew1: 0, ew2: 0, count: 0, texture: 0, drill: 0, moduleId: 0 }
...
export default function EditModuleDetails() {
    ...
    const { name, matIndex, length, width, count, grooveId, commentId, el1, el2, ew1, ew2, texture, drill } = details.find(d => d.id === selectedId) || emptyDetail
    const heads = [{ caption: 'id', sorted: true }, { caption: 'Наименование', sorted: true }, { caption: 'Инд. мат.' }, { caption: 'Длина' }, { caption: 'Ширина' }, { caption: 'Кол-во' }, { caption: 'Текстура' }, { caption: 'Сверловка' }, { caption: 'Кромка Д1' }, { caption: 'Кромка Д2' }, { caption: 'Кромка Ш1' }, { caption: 'Кромка Ш2' }, { caption: 'Паз' }, { caption: 'Примечание' }]
    const contents: TableDataRow[] = details.map(d => ({ key: d.id, data: [d.id, d.name, d.matIndex, d.length, d.width, d.count, d.texture ? "ДА" : "НЕТ", d.drill ? "ДА" : "НЕТ", edges.get(d.el1)?.name, edges.get(d.el2)?.name, edges.get(d.ew1)?.name, edges.get(d.ew2)?.name, grooves.get(d.grooveId), comments.get(d.commentId)] }))
    const editItems: EditDataItem[] = [
        ...
        { title: "Индекс материала:", value: matIndex, inputType: InputType.TEXT, propertyType: PropertyType.INTEGER_POSITIVE_NUMBER, checkValue: value => ({ success: (value as number) > 0, message: "Укажите индекс материала" }) },
        ...
        { title: "Сверловка:", value: !!drill, inputType: InputType.CHECKBOX },
        ...
        {(perm?.Read) ? <EditDataSection items={editItems}
            onUpdate={perm?.Update ? {
                ...
                onAction: async (values) => {
                    ...
                    const drill =values[6]?1:0
                    const el1 =values[7] as number
                    const el2 =values[8] as number
                    const ew1 =values[9] as number
                    const ew2 =values[10] as number
                    const grooveId =values[11] as number
                    const commentId =values[12] as number
                    const result = await updateModuleDetail({ id: selectedId, moduleId: selectedModuleId, name, matIndex, length, width, count, texture, drill, el1, el2, ew1, ew2, grooveId, commentId })
            ...
            onAdd={perm?.Create ? {
                ...
                onAction: async (values) => {
                    ...
                    const grooveId = values[11] as number
                    const commentId = values[12] as number
                    const result = await addModuleDetail({ moduleId: selectedModuleId, name, matIndex, length, width, count, texture, drill, el1, el2, ew1, ew2, grooveId, commentId })
                    if (result.success) loadData(selectedModuleId)
                    return result
                }
            } : undefined} /> : <div></div>}
